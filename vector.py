...
class Vector(object):
  def __init__(self, cache_path='.vector_cache',
      vector_type='fasttext.16B.subword', unk_init=torch.Tensor.zero_):
    urls = {
        'fasttext.16B.subword': 'https://s3-us-west-1.amazonaws.com/fasttext-vectors/wiki-news-300d-1M-subword.vec.zip',
        'fasttext.16B':         'https://s3-us-west-1.amazonaws.com/fasttext-vectors/wiki-news-300d-1M.vec.zip',
        'fasttext.600B':        'https://s3-us-west-1.amazonaws.com/fasttext-vectors/crawl-300d-2M.vec.zip',
    }
  ...
  def _prepare(self, vocab):
    ...
    for token, idx in vocab.items():
      if self.check(token): word2vec[idx, :] = self[token]
      else: word2vec[idx, :].normal_(-0.01, 0.01)

    word2vec[0, :] = self.unk_init(torch.Tensor(1, self.dim)) # fill zero for unknown token
    return word2vec
  ...
  def cache(self, name, cache_path, url=None):
    ...
    if not osp.isfile(path_pt):
      ...
      # Read meta-data of vector files
      ...
      for line in tqdm(lines, total=len(lines)):
        ...
        vectors.extend(map(float, entries))
        itos.append(word)
